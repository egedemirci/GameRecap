...
export default class gameController {
  static async getAllGames(req, res, next) {
    try {
      const results = await db.query("SELECT * FROM game_recap.onlinestores");
      res.status(200).json({
  ...
  static async deleteById(req, res, next) {
    try {
      const results = await db.query(
        "DELETE FROM game_recap.onlinestores WHERE store_id = $1 returning *",
        [req.params.id]
      );
  ...
  static async createGame(req, res, next) {
    try {
      const newGame = await db.query(
        "INSERT INTO game_recap.onlinestores (store_name) values ($1) returning *",
        [req.body.store_name]
      );
      res.status(200).json({
        data: newGame.rows[0],
  ...
  static async getGamesByDate(req, res, next) {
    try {
        const result = await db.query(
          `SELECT * FROM game_recap.onlinestores `

        )
        const filteredRows = result.rows.filter(row => row.store_name.toLowerCase().startsWith(req.body.filter.toLowerCase()));
  ...
  static async getGameById(req, res, next) {
    try {
      const results = await db.query(
        "SELECT * FROM game_recap.onlinestores WHERE store_id = $1",
        [req.params.store_id]
      );
      if (results.rows.length == 0) {
        throw {
