...
export default function MainPage(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await GameFinder.get("/dlc");
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    try {
      const response = await GameFinder.delete(`/dlc/${id}`);
      setGames(
        games.filter((game) => {
          ...
          <Container maxWidth="sm">
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 100 }} aria-label="customized table">
                <TableHead>
                  <TableRow>
                    <StyledTableCell align="right">Dlc ID</StyledTableCell>
                    <StyledTableCell align="right">Dlc Name</StyledTableCell>
                ...
                <TableBody>
                  {games.map((game) => (
                    <StyledTableRow key={game.dlc_id}>
                      <StyledTableCell align="right">
                        {game.dlc_id}
                      </StyledTableCell>
                      <StyledTableCell align="right">
